<template>
 <div>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection" :searchInfo="searchInfo">
      ...
      <MzVillageInfoModal @register="registerModal" @success="handleSuccess"/>
   </div>
</template>
...
<script lang="ts" setup>
  ...
  import {downloadFile} from '/@/utils/common/renderUtils';
    ...
    const {prefixCls, tableContext, onImportXls, onExportXls} = useListPage({
        tableProps: {
            api: mzVillageInfoList,
            columns: mzVillageInfoColumns,
            canResize: false,
            useSearchForm: false,
            actionColumn: {
                width: 180,
        ...
        exportConfig: {
            name: '民政小区信息',
            url: mzVillageInfoExportXlsUrl,
            params: {
                'mzCommunityId': mainId
            }
        },
        importConfig: {
            url: ()=>{
                return mzVillageInfoImportUrl + '/' + unref(mainId)
            }
        }
    });
    ...
    watch(mainId, () => {
         searchInfo['mzCommunityId'] = unref(mainId);
         reload();
      }
    );
   ...
   function handleDetail(record: Recordable) {
    ...
    async function handleDelete(record) {
        await mzVillageInfoDelete({id: record.id}, handleSuccess);
    }
